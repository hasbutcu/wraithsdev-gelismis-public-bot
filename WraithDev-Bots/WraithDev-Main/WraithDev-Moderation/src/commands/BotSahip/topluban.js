...
module.exports = {
  conf: {
    aliases: [],
    name: "topluban",
    help: "topluban <wraithsdev/ID> <mac/ID>",
    category: "sahip",
    owner: true,
  },
  ...
  run: async (client, message, args) => {
    ...
    await ozicik.react(wraithsdevTik);

    const filter = (reaction, user) => {
        return reaction.emoji.name === 'wraithsdevTik' && user.id === message.author.id;
    };
    const collector = ozicik.createReactionCollector({ filter, time: 10000 });

    collector.on("collect", async () => {
        await ozicik.edit({ content: `${wraithsdevTik} ${members.length} adet kullanıcı başarıyla yasaklandı.`})
