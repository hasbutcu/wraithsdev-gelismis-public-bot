...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    let arr = [];
    if (!zaman) return message.reply({ content: `${wraithsdevRed} Lütfen komutu doğru kullanın! \`.çekiliş 10m 1 Spotify\`` })
    if (!kazanan) return message.reply({ content: `${wraithsdevRed} Lütfen komutu doğru kullanın! \`.çekiliş 10m 1 Spotify\`` })
    if (isNaN(kazanan)) return message.reply({ content: `${wraithsdevRed} Lütfen komutu doğru kullanın! \`.çekiliş 10m 1 Spotify\`` })
    if (kazanan > 1) return message.reply({ content: `${wraithsdevRed} Şuanlık sadece 1 kazanan belirleyebilirsiniz!` })
    if (!odul) return message.reply({ content: `${wraithsdevRed} Lütfen komutu doğru kullanın! \`.çekiliş 10m 1 Spotify\`` })
    let sure = ms(zaman)
    ...
    if (message) message.delete();
      ...
      .setEmoji(`${wraithsdevCekilis}`)
    
      const row = new ActionRowBuilder()
      .addComponents([ xd ]);

let msg = await message.channel.send({ content: `${wraithsdevCekilis} **ÇEKİLİŞ** ${wraithsdevCekilis}`,
embeds: [
new EmbedBuilder()
.setTitle(`${odul}`)
.setFooter({ text : `Kazanacak kişi sayısı: ${kazanan} | Bitiş Süresi • ${moment(kalan).format("LLL")}` })
.setDescription(`
Katılmak için ${wraithsdevCekilis} tıklayın!
...
.setDescription(`
${wraithsdevCekilis} Çekiliş Sonuçlandı!
