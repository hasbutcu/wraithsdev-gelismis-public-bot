...
const { wraithsdevRed, wraithsdevTik, secret1, secret2, secret3, secret4, secret5, secret6, secret7, secret8, secret9, secret10, wraithsdevOk } = require("../../../../src/configs/emojis.json");
...
const Channel = Guild?.channels.cache.get(room?.ID);
    ...
    collector.on('end', i => {
       msg.delete();
        ...
        await interaction.reply({ content: `Lütfen kanalınıza girmesini istediğiniz üye(leri) etiketleyiniz. \`Örn: @wraithsdev\``, ephemeral: true })
       ...
       let yarak = await interaction.channel.send({ content: `Odaya erişimini kaldırmak istediğiniz üyeleri listeden seçiniz.`, components: [row]});
            ...
            yarak.awaitMessageComponent({ filter, componentType: ComponentType.StringSelect, time: 60000 }).then((x) => {
                ...
                interaction.channel.send({ content: `${wraithsdevTik} Başarılı bir şekilde <@${x.values}> adlı üyenin odanıza erişim izni silindi.` });
                });
        ...
        await interaction.reply({ content: `Kanal sahipliğini aktarmak istediğiniz üyeyi etiketleyiniz. \`Örn: @wraithsdev\``, ephemeral: true })
        interaction.channel.awaitMessages({ filter, max: 1 }).then(async function (collected) {
