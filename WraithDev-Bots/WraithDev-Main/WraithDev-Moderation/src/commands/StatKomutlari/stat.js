...
const { wraithsdevStar, wraithsdevSonsuz, wraithsdevOk } = require("../../../../src/configs/emojis.json");
...
const canvas = Canvas.createCanvas(930, 295);
  ...
  let background = await Canvas.loadImage("https://i.imgur.com/j4puvuI.png");
  ctx.save();
...
if (liste.length > 0) {
embeds.addFields({ name:`${wraithsdevStar} **Sesli Kanal İstatistiği**`, value: `
${liste.slice(page == 1 ? 0 : page * 10 - 10, page * 10).join('\n')}
`, inline: false })
}
embeds.addFields({ name:`${wraithsdevStar} **Sesli Kategori İstatistiği**`, value: `
${wraithsdevSonsuz} Toplam: \`${moment.duration(voiceData ? voiceData.topStat : 0).format("H [saat], m [dakika]")}\`
${wraithsdevSonsuz} Public Odalar: \` ${await category(conf.publicParents)} \`
${wraithsdevSonsuz} Secret Odalar: \` ${await category(conf.privateParents)} \`
${wraithsdevSonsuz} Alone Odalar: \` ${await category(conf.aloneParents)} \`
${wraithsdevSonsuz} Yönetim Yetkili Odaları: \` ${await category(conf.funParents)} \`
${wraithsdevSonsuz} Kayıt Odaları: \` ${await category(conf.registerParents)} \`
...
.addFields({ name:`${wraithsdevStar} **Mesaj İstatistiği**`, value: `
...
${wraithsdevStar} \`[ ${regular} gerçek, ${bonus} ekstra, ${leave} ayrılmış, ${fake} sahte ]\`
      
${wraithsdevOk} Günlük: \` ${daily} \`, Haftalık: \` ${weekly} \`, Taglı: \` ${tagged} \`
