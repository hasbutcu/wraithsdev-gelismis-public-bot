...
${wraithsdevRed} üyesinin toplamda **${data ? `${data.names.length}` : "0"}** isim kayıtı bulundu
${data ? data.names.splice(-3).map((x, i) => `\` ${i + 1} \` \` ${x.name} \` ${x.sebep ? `(${x.sebep})` : ""} ${x.rol ? `(${x.rol})` : ""}`).join("\n") : "Daha önce kayıt olmamış."}

Üyesinin önceki isimlerine \`.isimler <@wraithsdev/ID>\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.
`)
...
if(button.customId === "MAN") {
  ...
  if(msg) msg.delete();
  message.reply({ embeds: [ozie], components: []});
...
if(msg) msg.delete();
button.reply({ embeds: [ozik], components: []});
...
if(msg) msg.delete();
button.reply({ content:`İşlem Başarıyla İptal Edildi ${wraithsdevTik}`, embeds: [], components: []});
