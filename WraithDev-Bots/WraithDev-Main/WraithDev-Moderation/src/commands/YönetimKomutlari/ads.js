...
const { wraithsdevRed, wraithsdevTik, wraithsdevRevu, wraithsdevOk } = require("../../../../src/configs/emojis.json")
  ...
  run: async (client, message, args, embed) => {
    ...
    .then(() => {
      message.channel.awaitMessages({
        ...
        message.reply({ content :`${wraithsdevTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle jaillendi! \`(Ceza ID: #${penal.id})\``});
        ...
        }).catch(err => message.reply({ content :`${wraithsdevRed} 2 dakika içerisinde kanıt ekran görüntüsü atılmadığı için iptal edildi.`}));
      });
