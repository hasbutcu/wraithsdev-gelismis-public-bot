...
module.exports = {
  conf: {
    ...
    name: "yetki-aldır",
    help: "yetki-aldır <wraithsdev/ID>",
    category: "yetkili",
  },
  ...
  run: async (client, message, args, embed) => {
    ...
    if (!conf.staffs.some(x => message.member.roles.cache.has(x))) return message.react(wraithsdevRed)
    ...
    return }
    if (!member.user.displayName.includes(conf.tag))
    {
    message.react(wraithsdevRed)
...
let collector = await msg.createMessageComponentCollector({ filter, time: 60000 })
    ...
    collector.on("collect", async (button) => {
      ...
      .setAuthor({ name: member.displayName, iconURL: member.user.avatarURL({ dynamic: true })})
      .setTimestamp()
      .setDescription(`${message.author}, ${member.toString()} Adlı kullanıcı senin yetki aldırma isteğini onayladı. ${wraithsdevTik}`)
      ...
      client.channels.cache.find(x => x.name == "yetki_log").wsend({ content:`${message.author} \`(${message.author.id})\` kişisi ${member} \`(${member.id})\` kişisini yetkiye aldı! ${wraithsdevTik}`})
      ...
      if(button.customId === "wraithsdevRed") {
        await button.deferUpdate();
      ...
      .setFooter({ text: message.author.tag, iconURL: message.author.avatarURL({ dynamic: true })})
      .setTimestamp()
      .setDescription(`${message.author}, ${member} Adlı kullanıcı senin yetki aldırma isteğini onaylamadı. ${wraithsdevRed}`)
