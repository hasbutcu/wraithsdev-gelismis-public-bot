...
const { wraithsdevKalp, wraithsdevStaff , wraithsdevd2, wraithsdevb2, wraithsdevd1, wraithsdevb3, wraithsdevd3, wraithsdevRed } = require("../../../../src/configs/emojis.json")
...
module.exports = {
    ...
    if(!conf.staffs.some(rol => member.roles.cache.has(rol))) return message.react(wraithsdevRed)
    ...
    `${currentRank ?`
    ${currentRank !== client.ranks[client.ranks.length-1] ? `Şu an ${Array.isArray(currentRank.role) ? `<@&${currentRank.role[0]}>` : `<@&${currentRank.role[0]}>`} rolündesiniz. ${Array.isArray(maxValuee.role) ? `<@&${maxValuee.role[0]}>` : `<@&${maxValuee.role[0]}>`} rolüne ulaşmak için \`${maxValuee.coin-coinData.coin}\` puan daha kazanmanız gerekiyor!` : "Şu an son yetkidesiniz! Emekleriniz için teşekkür ederiz. :)"}` : ` 
    Şuan ${message.member.roles.highest} rolündesiniz. ${Array.isArray(maxValuee.role) ? `<@&${maxValuee.role[0]}>` : `<@&${maxValuee.role[0]}>`} rolüne ulaşmak için \`${maxValuee.coin - (coinData ? coinData.coin : 0)}\`  Puan daha kazanmanız gerekiyor!`}` : ""
...
`**İnvite Görev Durumu :** 
${wraithsdevStaff} ${progressBar(gorevData ? gorevData.invite : 0, 10, 10)} \`${total} (${total}/10)\` 
` : "";
          //
...
`**Kayıt Görev Durumu :**  
${progressBar(kayitgData ? kayitgData.kayit : 0, 10, 10)} \`${kayittotal} (${kayittotal}/10)\`
` : "";
          //
...
`**Mesaj Görev Durumu :**  
${progressBar(mesajData ? mesajData.mesaj : 0, 500, 5)} \`${mesajtotal} (${mesajtotal}/500)\`
` : "";
          //
...
`**Taglı Üye Durumu :**  
${wraithsdevKalp} ${progressBar(tagData ? tagData.tagli : 0, 5, 5)} \`${tagTotal} (${tagTotal}/5)\`
` : "";


message.channel.send({ embeds: [embed.setDescription(`
${member.toString()}, (${member.roles.highest}) rolüne ait görevlerin aşağıda belirtilmiştir. Görevler tamamlandığında tamamladığın görevler otomatik olarak coinlerine eklenicektir.  

Kalan Süre: \`${moment.duration(moment().endOf('day').valueOf() - Date.now()).format("H [saat], m [dakika] s [saniye]")}\`
4 görevi tamamlamak sana toplam \`120 Coin\` verecektir!
          
${coinStatus1} **Ödül :**  \`30\` Coin\n
${coinStatus2} **Ödül :**  \`30\` Coin\n
${coinStatus3} **Ödül :**  \`30\` Coin\n
${coinStatus4} **Ödül :**  \`30\` Coin
`)]})
}
};
function progressBar(value, maxValue, size) {
    ...
    const progressText = wraithsdevd2.repeat(progress);
    const emptyProgressText = wraithsdevb2.repeat(emptyProgress);
    
    return emptyProgress > 0 ? wraithsdevd1+progressText+emptyProgressText+wraithsdevb3 : wraithsdevd1+progressText+emptyProgressText+wraithsdevd3;
