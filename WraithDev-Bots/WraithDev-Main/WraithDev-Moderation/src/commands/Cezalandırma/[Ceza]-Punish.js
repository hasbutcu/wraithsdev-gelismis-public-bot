...
const { wraithsdevRed, wraithsdevTik, wraithsdevSaat, wraithsdevOk, wraithsdevCeza, wraithsdevRevu } = require("../../../../src/configs/emojis.json")
...
msg.edit({ content :`${wraithsdevTik} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **kalıcı olarak** banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

const log = new EmbedBuilder() 
.setDescription(`
${wraithsdevCeza} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
${wraithsdevRevu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
${wraithsdevOk} Ban Sebebi: \`${reason}\`
`)
...
if(button.customId === "üç") {
  ...
  msg.edit({ content :`${wraithsdevTik} ${member ? member.toString() : member.user.username} üyesi, ${message.author} tarafından, \`Reklam\` nedeniyle banlandı! \`(Ceza ID: #${penal.id})\``, embeds: [], components: []})

  const log = embed
  .setDescription(`
${wraithsdevCeza} Banlanan Üye: **${member ? member.toString() : member.user.username}** \`${member.user.id}\`
${wraithsdevRevu} Banlayan Yetkili: ${message.author} \`${message.author.id}\`
${wraithsdevOk} Ban Sebebi: \`Reklam\`
  `)
  .setFooter({ text:`${moment(Date.now()).format("LLL")}    (Ceza ID: #${penal.id})` })
  message.guild.channels.cache.get(conf.banLogChannel).send({ embeds: [log]});
