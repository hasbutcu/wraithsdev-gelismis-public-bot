...
module.exports = {
  conf: {
    ...
    name: "vmute",
    help: "vmute <wraithsdev/ID> <Süre> <Sebep>",
  ...
  run: async (client, message, args, embed) => {
    ...
    if (duration) {
      ...
      if(msg) msg.delete();
      await message.channel.send({ content:`${wraithsdevTik} ${member.toString()} üyesi, ${message.author} tarafından, \`${reason}\` nedeniyle **sesli kanallarda** susturuldu! \`(Ceza ID: #${penal.id})\``}).then((e) => setTimeout(() => { e.delete(); }, 50000));
       message.react(wraithsdevTik)
    ...
    await interaction.deferUpdate();
    if(msg) msg.delete();
    interaction.followUp({ content: `${wraithsdevTik} Sesli Susturma işlemi başarıyla iptal edildi.`, ephemeral: true });
    }
    })
    }
      },
