...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.channel.send({ content:"Yeterli yetkin bulunmuyor wraithsdev ile görüş !"}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    await penals.find({ guildID: message.guild.id, userID: member.user.id, }).sort({ date: -1 }).exec(async (err, res) => {
        ...
        if (data.length === 0) return message.channel.send({ content:`${wraithsdevTik} ${member.toString()} üyesinin sicili temiz!`}).then((e) => setTimeout(() => { e.delete(); }, 5000));
