...
const { wraithsdevStar, wraithsdevTik, wraithsdevRed } = require("../../../src/configs/emojis.json")
...
.setDescription(`
${wraithsdevRed} ${member.toString()} isimli eski taglımız, tagımızı <t:${Math.floor(Date.now() / 1000)}:R> bıraktı.
...
.setDescription(`
${wraithsdevTik} ${member.toString()} isimli üye ailemize katıldı, tagımızı <t:${Math.floor(Date.now() / 1000)}:R> aldı.

\` ➥ \` İsim Değişikliği: \` ${oldUser.displayName} \` => **${newUser.displayName}**
\` ➥ \` Anlık taglı üye: **${guild.members.cache.filter(x => x.user.displayName.includes(conf.tag)).size}**
`);
channel.wsend({ content: `${member.toString()} [\` ${member.id} \`]`, embeds: [embed]});
kanal.wsend({ content: `${member.toString()} üyesi ${conf.tag} tagımızı alarak ailemize katıldı! Ailemiz ${guild.members.cache.filter(x => x.user.displayName.includes(conf.tag)).size} kişi oldu!` });
}
