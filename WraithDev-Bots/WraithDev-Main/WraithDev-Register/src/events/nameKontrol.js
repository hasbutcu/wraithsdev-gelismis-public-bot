...
module.exports = async (oldMember, newMember) => {
    ...
    if (executor.bot) return;
        ...
        \` ➥ \` İsim Değişikliği: \` ${oldMember.nickname ? oldMember.nickname : oldMember.user.displayName} \` => **${newMember.nickname ? newMember.nickname : newMember.user.displayName}**
        \` ➥ \` Değiştiren Yetkili: \` ${executor.tag} \`
        \` ➥ \` Değiştirme Tarihi: <t:${Math.floor(Date.now() / 1000)}:R>
        `);

    if (newMember.guild.channels.cache.find(c => c.name === "name_log")) newMember.guild.channels.cache.find(c => c.name === "name_log").send({ embeds: [ozi] });
    const registerData = await isimler.findOne({ guildID: newMember.guild.id, memberID: member.id });
    if (!registerData) new isimler({
        guildID: newMember.guild.id,
        memberID: member.id
    }).save().catch(() => { });
    await isimler.findOneAndUpdate({ guildID: newMember.guild.id, userID: newMember.id }, { $push: { names: { name: newMember.nickname || newMember.user.displayName, yetkili: executor.id, sebep: "Sağ-Tık İsim Değiştirme", date: Date.now() } } }, { upsert: true });
}; 
module.exports.conf = {
